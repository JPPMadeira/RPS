...
function detectDragAndDrop() { // Permite agarrar e largar as imagens
...
function handleDragStart(e) { // Permite arrastar as imagens
...
function handleDragEnd(e) { // Dragend é acionado quando se acaba de arrastar a imagem largando o mouse
...
function handleDragEnter(e) { // Durante a operação de arrastar impede atualizações
...
function handleDrop(e) { // Permite largar as figuras
...
function getRPSFooter(e) { // Configura o rodapé
...
function getRPSType(e) { // Serve para ver o tipo de dados que estão armazenados na variável
    var footer = getRPSFooter(e);
    if(footer) return footer.innerHTML;
    else return undefined;
}

function swapRPS(a, b) { // A função recebe dois valores e troca-os
    var holding = Object();
...
function init() {
    if((dndSupported = detectDragAndDrop())) { // Permite agarrar e largar as imagens
